...
module WalleeRubySdk
  class ApiClient
    ...
    def call_api(http_method, path, opts = {}, connection_timeout)
      stream = nil
      begin
        Faraday::NestedParamsEncoder.sort_params = false
        response = connection(opts).public_send(http_method) do |req|
          request = build_request(http_method, path, req, opts, connection_timeout)
    ...
    def build_request(http_method, path, request, opts = {}, connection_timeout)
      ...
      bearer_token = get_authentication_headers(http_method, path, query_params)
      header_params = header_params.merge({ "Authorization" => "Bearer #{bearer_token}" })
      ...
      request.options.timeout        = connection_timeout    || config.timeout
    ...
    def convert_to_type(data, return_type)
      ...
      when /\AHash\<String, (.+)\>\z/
        ...
        klass = WalleeRubySdk.const_get(return_type)
    ...
    # @param [String] http_method HTTP method/verb (e.g. POST)
    # @param [String] path URL path (e.g. /account/new)
    # @return [Hash{String => String}] A Bearer token as the value.
    def get_authentication_headers(http_method, path, query_params)
      base_url = URI(@config.base_url)
      completed_path = "#{base_url.path}#{path}"
      encoded_query_params = Faraday::NestedParamsEncoder.encode(query_params) unless query_params.empty?
      uri = encoded_query_params ? "#{completed_path}?#{encoded_query_params}" : completed_path

      payload = {
        sub: @config.user_id.to_s,
        iat: Time.now.to_i,
        requestPath: uri,
        requestMethod: http_method.upcase
      }
      ...
      JWT.encode(payload, decoded_secret, 'HS256', custom_headers)
    end
    ...
    # @return [String] the Accept header (e.g. application/json,text/plain)
    ...
    def build_collection_param(param, collection_format)
      ...
      when :multi
        ...
        # For Faraday we have to handle it
        if param.is_a?(String)
          return param
        elsif param.is_a?(Array) && param.empty?
          return nil
        end
        param.join(',')
      else
