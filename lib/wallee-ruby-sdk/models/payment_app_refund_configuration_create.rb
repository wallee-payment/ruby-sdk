  ...
  class PaymentAppRefundConfigurationCreate
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::PaymentAppRefundConfigurationCreate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::PaymentAppRefundConfigurationCreate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'refund_timeout_in_minutes')
        self.refund_timeout_in_minutes = attributes[:'refund_timeout_in_minutes']
      end

      if attributes.key?(:'multiple_refunds_supported')
        self.multiple_refunds_supported = attributes[:'multiple_refunds_supported']
      end

      if attributes.key?(:'refund_endpoint')
        self.refund_endpoint = attributes[:'refund_endpoint']
      end
    end
    ...
    def hash
      [refund_timeout_in_minutes, multiple_refunds_supported, refund_endpoint].hash
