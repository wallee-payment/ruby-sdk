  ...
  class SubscriptionProductComponentReference
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::SubscriptionProductComponentReference` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::SubscriptionProductComponentReference`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@sku.nil? && @sku !~ Regexp.new(/([0-9a-zA-Z\-_]+)/)
      true
    end
    ...
    def hash
      [linked_space_id, name, planned_purge_date, id, state, sku, version].hash
    end
