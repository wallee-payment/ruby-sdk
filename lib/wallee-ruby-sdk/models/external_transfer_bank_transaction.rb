  ...
  class ExternalTransferBankTransaction
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::ExternalTransferBankTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::ExternalTransferBankTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'external_bank_name')
        self.external_bank_name = attributes[:'external_bank_name']
      end
      ...
      if attributes.key?(:'external_account_identifier')
        self.external_account_identifier = attributes[:'external_account_identifier']
      end
    ...
    def hash
      [external_bank_name, linked_space_id, external_account_identifier, id, bank_transaction, version, external_account_type].hash
