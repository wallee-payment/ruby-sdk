  ...
  class SubscriptionSuspension
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::SubscriptionSuspension` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::SubscriptionSuspension`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'effective_end_date')
        self.effective_end_date = attributes[:'effective_end_date']
      end
      ...
      if attributes.key?(:'period_bill')
        self.period_bill = attributes[:'period_bill']
      end
      ...
      if attributes.key?(:'end_action')
        self.end_action = attributes[:'end_action']
      end
    ...
    def hash
      [effective_end_date, note, reason, period_bill, planned_purge_date, language, subscription, created_on, version, planned_end_date, linked_space_id, end_action, id, state].hash
    end
