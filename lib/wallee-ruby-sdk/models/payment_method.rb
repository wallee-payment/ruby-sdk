  ...
  class PaymentMethod
    ...
    def self.openapi_types
      {
        ...
        :'data_collection_types' => :'Array<DataCollectionType>',
        :'image_path' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::PaymentMethod` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::PaymentMethod`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'data_collection_types')
        if (value = attributes[:'data_collection_types']).is_a?(Array)
          self.data_collection_types = value
        end
      end
      ...
      if attributes.key?(:'merchant_description')
        if (value = attributes[:'merchant_description']).is_a?(Hash)
    ...
    # Custom attribute writer method with validation
    # @param [Object] data_collection_types Value to be assigned
    def data_collection_types=(data_collection_types)
      if data_collection_types.nil?
        fail ArgumentError, 'data_collection_types cannot be nil'
      end

      @data_collection_types = data_collection_types
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          data_collection_types == o.data_collection_types &&
    ...
    def hash
      [supported_currencies, data_collection_types, image_path, name, description, merchant_description, id].hash
    end
