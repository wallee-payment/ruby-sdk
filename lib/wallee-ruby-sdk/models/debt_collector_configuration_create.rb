...
module WalleeRubySdk
  ...
  class DebtCollectorConfigurationCreate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::DebtCollectorConfigurationCreate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::DebtCollectorConfigurationCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] collector Value to be assigned
    def collector=(collector)
      if collector.nil?
        fail ArgumentError, 'collector cannot be nil'
      end
    ...
    def hash
      [skip_review_enabled, name, enabled_space_views, state, conditions, priority, collector].hash
    end
