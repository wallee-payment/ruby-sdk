  ...
  class ConnectorInvocation
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::ConnectorInvocation` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::ConnectorInvocation`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'time_took_in_milliseconds')
        self.time_took_in_milliseconds = attributes[:'time_took_in_milliseconds']
      end
    ...
    def hash
      [linked_space_id, time_took_in_milliseconds, stage, planned_purge_date, id, created_on, version, transaction].hash
    end
