...
module WalleeRubySdk
  class PaymentMethodConfigurationCreate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::PaymentMethodConfigurationCreate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::PaymentMethodConfigurationCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] data_collection_type Value to be assigned
    def data_collection_type=(data_collection_type)
      if data_collection_type.nil?
        fail ArgumentError, 'data_collection_type cannot be nil'
      end
