  ...
  class MetricUsage
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::MetricUsage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::MetricUsage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'metric_description')
        if (value = attributes[:'metric_description']).is_a?(Hash)
          self.metric_description = value
        end
      end

      if attributes.key?(:'metric_name')
        if (value = attributes[:'metric_name']).is_a?(Hash)
    ...
    def hash
      [consumed_units, metric_description, metric_name, metric_id].hash
