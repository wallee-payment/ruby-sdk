  ...
  class WebhookUrl
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::WebhookUrl` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::WebhookUrl`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@url.nil? && @url.to_s.length < 9
        invalid_properties.push('invalid value for "url", the character length must be greater than or equal to 9.')
      end
    ...
    def url=(url)
      ...
      if url.to_s.length < 9
        fail ArgumentError, 'invalid value for "url", the character length must be greater than or equal to 9.'
      end
    ...
    def hash
      [linked_space_id, application_managed, name, planned_purge_date, id, state, version, url].hash
