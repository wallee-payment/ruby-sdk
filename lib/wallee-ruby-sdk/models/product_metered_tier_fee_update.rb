  ...
  class ProductMeteredTierFeeUpdate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::ProductMeteredTierFeeUpdate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::ProductMeteredTierFeeUpdate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'start_range')
        self.start_range = attributes[:'start_range']
      end

      if attributes.key?(:'metered_fee')
        self.metered_fee = attributes[:'metered_fee']
      end
    ...
    def hash
      [start_range, metered_fee, fee, version].hash
    end
