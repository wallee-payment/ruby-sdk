  ...
  class Customer
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::Customer` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::Customer`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@customer_id.nil? && @customer_id !~ Regexp.new(/[	\x20-\x7e]*/)
      true
    end
    ...
    def hash
      [linked_space_id, meta_data, email_address, family_name, given_name, preferred_currency, customer_id, language, id, created_on, version].hash
    end
