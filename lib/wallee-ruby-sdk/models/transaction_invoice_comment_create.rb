...
module WalleeRubySdk
  class TransactionInvoiceCommentCreate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::TransactionInvoiceCommentCreate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::TransactionInvoiceCommentCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @transaction_invoice.nil?
        invalid_properties.push('invalid value for "transaction_invoice", transaction_invoice cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @transaction_invoice.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] transaction_invoice Value to be assigned
    def transaction_invoice=(transaction_invoice)
      if transaction_invoice.nil?
        fail ArgumentError, 'transaction_invoice cannot be nil'
      end
    ...
    def hash
      [content, transaction_invoice].hash
    end
