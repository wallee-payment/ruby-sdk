  ...
  class PaymentLinkActive
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::PaymentLinkActive` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::PaymentLinkActive`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [shipping_address_handling_mode, allowed_redirection_domains, language, version, available_from, line_items, available_until, name, currency, maximal_number_of_transactions, allowed_payment_method_configurations, applied_space_view, billing_address_handling_mode, state].hash
