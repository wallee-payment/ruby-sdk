  ...
  class ApplicationUserCreate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WalleeRubySdk::ApplicationUserCreate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::ApplicationUserCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] primary_account Value to be assigned
    def primary_account=(primary_account)
      if primary_account.nil?
        fail ArgumentError, 'primary_account cannot be nil'
      end
    ...
    def hash
      [request_limit, name, state, primary_account].hash
    end
