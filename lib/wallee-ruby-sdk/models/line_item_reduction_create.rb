...
module WalleeRubySdk
  class LineItemReductionCreate
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WalleeRubySdk::LineItemReductionCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] quantity_reduction Value to be assigned
    def quantity_reduction=(quantity_reduction)
      if quantity_reduction.nil?
        fail ArgumentError, 'quantity_reduction cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] unit_price_reduction Value to be assigned
    def unit_price_reduction=(unit_price_reduction)
      if unit_price_reduction.nil?
        fail ArgumentError, 'unit_price_reduction cannot be nil'
      end

      @unit_price_reduction = unit_price_reduction
    end
